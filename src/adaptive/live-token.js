...
export async function issueLiveCaptureToken(secret, ttlMs = 90000) {
  const ttl = Math.max(30000, Math.min(300000, Number(ttlMs) || 90000));
  const now = Date.now();
  const payload = {
    type: "m21_live_browser_probe",
    sid: crypto.randomUUID(),
    iat: now,
    exp: now + ttl,
  };
  const body = textToB64url(JSON.stringify(payload));
  const token = `${body}.${await signText(secret, body)}`;
  return { token, payload };
}

export async function verifyLiveCaptureToken(secret, token) {
  try {
    ...
    if (payload?.type !== "m21_live_browser_probe") return null;
