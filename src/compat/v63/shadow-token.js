...
export async function issueShadowBrowserToken(secret, ttlMs = 300000) {
  const ttl = Math.max(30000, Math.min(600000, Number(ttlMs) || 300000));
  const payload = {
    type: "m1_browser_probe",
...
export async function verifyShadowBrowserToken(secret, token) {
  try {
    ...
    if (payload?.type !== "m1_browser_probe") return null;
